 ...
 * {@code last} to retrieve the first element, and then {@code remove} to remove it after
 * successful processing. If the system crashes after {@code last} and during processing, the
...
public class QueueFile implements Closeable {
    ...
    /**
     * Reads the newest element. Returns null if the queue is empty.
     */
    public synchronized byte[] last() throws IOException {
        if (isEmpty()) return null;
        int length = last.length;
        byte[] data = new byte[length];
        ringRead(last.position + Element.HEADER_LENGTH, data, 0, length);
        return data;
    }

    /**
     * Invokes {@code visitor} with the newest element, if an element is available.
     */
    public synchronized void last(ElementVisitor visitor) throws IOException {
        if (elementCount > 0) {
            visitor.read(new ElementInputStream(last), last.length);
        }
    }
    ...
    public synchronized int forEach(ElementVisitor visitor) throws IOException {
        ...
        for (int i = 0; i < elementCount; i++) {
            Element current = readElement(position);
            boolean shouldContinue = visitor.read(new ElementInputStream(current), current.length);
            if (!shouldContinue) {
                return i + 1;
            }
            position = wrapPosition(current.position + Element.HEADER_LENGTH + current.length);
    ...
    /**
     * Resizes the queue to have n eldest elements.
     */
    public synchronized void resize(int n) throws IOException {
        if (isEmpty()) {
            return;
        }
        if (n < 0) {
            throw new IllegalArgumentException("Cannot have negative number of elements.");
        }
        if (n == 0) {
            clear();
            return;
        ...
        if (n > elementCount) {
            throw new IllegalArgumentException("Cannot grow queue");
        }
        ...
        for (int i = 0; i < n; i++) {
            newLast = readElement(position);
            totalLength += Element.HEADER_LENGTH + newLast.length;
            position = wrapPosition(newLast.position + Element.HEADER_LENGTH + newLast.length);
        }
